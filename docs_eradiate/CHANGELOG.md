# Eradiate fork of Mitsuba 3 — Changelog
...
- **`eovolpath` integrator**: New volumetric path integrator that replicates and
  extends the upstream `volpath` integrator. Introduces the *DDIS* (Decomposition
  Direct Importance Sampling) phase-sampling method for improved variance
...
- **Extremum plugins** (`extremum_grid`, `extremum_spherical`): New plugin
  category that computes local majorant/minorant bounds for a medium, enabling
...
- Corrected formatting issues and added missing docstrings across the extremum
  plugins and associated Python bindings.
