...
Analytical rectangle (:monosp:`arectangle`)
-------------------------------------------
...
class AnalyticalRectangle final : public Shape<Float, Spectrum> {
public:
    ...
    AnalyticalRectangle(const Properties &props) : Base(props) {
        ...
        m_shape_type = ShapeType::AnalyticalRectangle;
    ...
    void traverse(TraversalCallback *cb) override {
        Base::traverse(cb);
        cb->put("to_world", *m_to_world.ptr(), ParamFlags::Differentiable | ParamFlags::Discontinuous);
    }
    ...
    PositionSample3f sample_position(Float time, const Point2f &sample,
        ...
        ps.p = m_to_world.value() * Point3f(sample.x() * 2.f - 1.f, sample.y() * 2.f - 1.f, 0.f);
        ps.n    = m_frame.n;
    ...
    SurfaceInteraction3f eval_parameterization(const Point2f &uv,
                                               uint32_t ray_flags,
                                               Mask active) const override {
        Point3f p = m_to_world.value() * Point3f(uv.x() * 2.f - 1.f, uv.y() * 2.f - 1.f, 0.f);
    ...
    SilhouetteSample3f sample_silhouette(const Point3f &sample,
        ...
        Vector3f world_edge_dir = to_world * Vector3f(edge_dir.x(), edge_dir.y(), 0.f);
    ...
    SilhouetteSample3f primitive_silhouette_projection(
        ...
        ss.silhouette_d = dr::normalize(to_world * Vector3f(edge_dir.x(), edge_dir.y(), 0.f));
    ...
    SurfaceInteraction3f compute_surface_interaction(const Ray3f &ray,
        ...
        const AffineTransform4f& to_world = m_to_world.value();
        const AffineTransform4f& to_object = m_to_world.value().inverse();
    ...
    void optix_prepare_geometry() override {
        if constexpr (dr::is_cuda_v<Float>) {
            ...
            OptixRectangleData data = { bbox(), m_to_world.scalar().inverse() };
    ...
    std::string to_string() const override {
        ...
        oss << "Rectangle[" << std::endl
            ...
            << "  frame = " << indent(m_frame) << "," << std::endl
            << "  surface_area = " << surface_area() << "," << std::endl
            << "  " << indent(get_children_string()) << std::endl
            << "]";
        return oss.str();
    }
...
MI_EXPORT_PLUGIN(AnalyticalRectangle)
