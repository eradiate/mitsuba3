...
assembled — typically by a :monosp:`overlap` medium — from its
...
public:
    ...
    using SegmentArr = std::array<ExtremumSegment, MAX_OVERLAPPING_VOLUMES>;
    ...
    ExtremumOverlap(const Properties &props) : Base(props) {
        ...
        if (m_children.empty())
            Throw("extremum_overlap requires at least one nested extremum "
                  "structure");
        if (m_children.size() > MAX_OVERLAPPING_VOLUMES)
            Throw("extremum_overlap: at most %zu nested extremum structures "
                  "are supported, got %zu", MAX_OVERLAPPING_VOLUMES, m_children.size());
    ...
    void build(const Volume */*volume*/) override {
        // no-op, nested structures get built by their respective media.
    }
    ...
    TrackingStateType traverse_extremum(
        ...
        struct LoopState {
            ...
            DRJIT_STRUCT(LoopState, segment, state, advance, active, t, segments)
        } ls = {
        ...
        // zero initialize the segments, if the ray does not traverse the
        // extremum bbox, will set the segment[i].maxt to inf, and won't be
        // queried again.
        for (size_t i = 0; i < n_children; ++i)
            segments[i] = ExtremumSegment(mint, mint, 0.f, 0.f);
        ...
        dr::tie(ls) = dr::while_loop(
            ...
            if (dr::any_or<true>(ls.advance)) {
                ...
                for (size_t i = 0; i < n_children; ++i) {
                    ...
                    auto &segments = ls.segments;

                    Float eps = dr::maximum(dr::abs(ls.t), 1.f) * math::RayEpsilon<Float>;
                    Mask get_next_segment = (ls.t + eps > segments[i].maxt);

                    if (dr::any_or<true>(get_next_segment)) {
                        segments[i] = child->next_segment(ray, ls.t, ls.active);
                    }

                    seg_maxt = dr::minimum(seg_maxt, segments[i].maxt);
