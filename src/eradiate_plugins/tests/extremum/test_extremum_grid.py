...
def test_sample_horizontal_homogeneous(variants_any_scalar, variants_any_llvm):
    ...
    ref_segment = mi.ExtremumSegment(tmin=0.25, tmax=0.5, majorant=0.5, minorant=0.5)
    ref_tau = 0.125
    ...
    ref_segment = mi.ExtremumSegment(tmin=1.25, tmax=1.5, majorant=0.5, minorant=0.5)
    ref_tau = 0.125
    res, tau_acc = extremum_struct.sample_segment(ray, mint, maxt, desired_tau, active)
    ...
    ref_segment = mi.ExtremumSegment(tmin=0.375, tmax=0.625, majorant=0.5, minorant=0.5)
    ref_tau = 0.1875
    res, tau_acc = extremum_struct.sample_segment(ray, mint, maxt, desired_tau, active)
    # Test ray starting outside of the grid
    assert dr.allclose(ref_tau, tau_acc)
...
def test_sample_horizontal_heterogeneous(variants_any_scalar, variants_any_llvm):
    ...
    res, tau_acc = extremum_struct.sample_segment(ray, mint, maxt, desired_tau, active)
    ref_segment = mi.ExtremumSegment(tmin=0.5, tmax=0.75, majorant=0.6, minorant=0.5)
    ref_tau = 0.15
    # Test ray starting at segment start
    assert_compare_segment(ref_segment, res)
    assert dr.allclose(ref_tau, tau_acc)
...
def test_sample_diagonal(variants_any_scalar, variants_any_llvm):
    ...
    res, tau_acc = extremum_struct.sample_segment(ray, mint, maxt, desired_tau, active)
    ref_segment = mi.ExtremumSegment(tmin=1.0, tmax=2.0, majorant=1.0, minorant=1.0)
    ref_tau = 0.5
    # Test ray starting at segment start
    assert_compare_segment(ref_segment, res)
    assert dr.allclose(ref_tau, tau_acc)
