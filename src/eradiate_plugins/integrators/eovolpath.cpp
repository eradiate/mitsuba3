...
public:
    ...
    EOVolumetricPathIntegrator(const Properties &props) : Base(props) {
        ...
        if ( m_ddis_threshold < 0.f || m_ddis_threshold > 1.f)
            Throw("`ddis_threshold` is outside range [0.,1.].");
            ...
            if (dr::any_or<true>(act_medium_scatter)) {
                ...
                if (dr::any_or<true>(active_e)) {
                    ...
                    if (dr::any_or<true>(perform_ddis)) {
                        ...
                        auto [_, ddis_phase_pdf] = phase->eval_pdf(phase_ctx, ddis_mei, ds.d, perform_ddis);
                
                        // Compute mixture pdf accounting for DDIS probability
                        dr::masked(phase_pdf, perform_ddis) = (1.f - m_ddis_threshold) * phase_pdf +
                ...
                if (dr::any_or<true>(perform_ddis)) {
                    ...
                    dr::masked(phase_weight, perform_ddis) =
                        natural_val / phase_pdf;
                }
