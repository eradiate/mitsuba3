...
public:
    ...
    MishchenkoOceanBSDF(const Properties &props) : Base(props) {
        ...
        m_ext_eta    = props.texture<Texture>("ext_ior", 1.f);
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        if constexpr (is_polarized_v<Spectrum>) {
            ...
            Vector3f n(0, 0, 1);
            Vector3f p_axis_in  = dr::normalize(dr::cross(-wo_hat, dr::normalize(dr::cross(n, -wo_hat)))),
                     p_axis_out = dr::normalize(dr::cross(wi_hat, dr::normalize(dr::cross(n, wi_hat))));

            dr::masked(p_axis_in, dr::any(dr::isnan(p_axis_in)))   = Vector3f(0, 1, 0);
            dr::masked(p_axis_out, dr::any(dr::isnan(p_axis_out))) = Vector3f(0, 1, 0);
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if constexpr (is_polarized_v<Spectrum>) {
            value = eval_mishchenko(n_air, n_water, -wo_hat, wi_hat, m_sigma2,
                                    shadowing);
            value *= Frame3f::cos_theta(wo_hat) / dr::Pi<Float>;
     ...
     */
    MuellerMatrix<UnpolarizedSpectrum> eval_mishchenko(const Complex2u &n_ext,
                                                       const Complex2u &n_water,
                                                       Vector3f wi, Vector3f wo,
                                                       const Float sigma2,
                                                       Mask shadowing) const {
        ...
        Float coeff =
            1.0 / (dr::pow(k_d.z(), 4.0f) * 4.0 * mu_i * mu_o * 2.0 * sigma2);
        const Float exp = dr::exp(-(k_d.x() * k_d.x() + k_d.y() * k_d.y()) 
                                  / (k_d.z() * k_d.z() * 2.0 * sigma2));
        ...
        if (dr::any_or<true>(shadowing)) {
            ...
            Float shadow = 1.0 / (1.0 + gamma_shadowing(mu_i, sigma2) + gamma_shadowing(mu_o, sigma2));
    ...
    Float gamma_shadowing(const Float mu, const Float sigma2) const {
        Float cot = mu / dr::sqrt(1.0f - mu * mu);
        Float result = 
            0.5f *
            (dr::sqrt(2.0f * (1.0f - mu * mu) / dr::Pi<Float>) 
             * dr::sqrt(sigma2) / mu 
             * dr::exp(-cot * cot / (2.0f * sigma2)) 
             - (1.0f - dr::erf(cot / dr::sqrt(2.0f * sigma2))));
        return result;
    }
