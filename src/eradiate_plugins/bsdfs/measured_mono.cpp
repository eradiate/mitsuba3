...
Measured material (:monosp:`measured_mono`)
-------------------------------------------
 ...
 * - wavelength
   - |float|
   - Specifies the wavelength at which the model is evaluated with monochromatic
     variants.
...
class MeasuredMono final : public BSDF<Float, Spectrum> {
public:
    ...
    MeasuredMono(const Properties &props) : Base(props) {
        m_components.push_back(BSDFFlags::GlossyReflection | BSDFFlags::FrontSide);
        m_flags = m_components[0];
        m_wavelength = props.get<ScalarFloat>("wavelength", 550.f);
        ...
        if (is_spectral) {
            ...
            Throw("Measurements in RGB format cannot be used with the measured_mono plugin");
        }
    ...
    std::tie(sample, std::ignore) = m_vndf.invert(u_m, params, active);
        ...
        for (size_t i = 0; i < dr::size_v<UnpolarizedSpectrum>; ++i) {
            Float params_spec[3] = { phi_i, theta_i,
                is_spectral_v<Spectrum> ? si.wavelengths[i] : Float(m_wavelength) };
    ...
    std::string to_string() const override {
        ...
        oss << "MeasuredMono[" << std::endl
            << "  filename = \"" << m_name << "\"," << std::endl
            << "  wavelength = " << m_wavelength << "," << std::endl
